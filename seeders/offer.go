...
func (cfg *SeedersConfig) seedOffers(authors []string) ([]string, error) {
	IDs := []string{
		uuid.New().String(),
		uuid.New().String(),
	}
	offers := []database.SeedOfferParams{
		{
			ID:           IDs[0],
			Status:       1,
	...
	for _, offer := range offers {
		err := cfg.DB.SeedOffer(cfg.Ctx, offer)
		if err != nil {
			return nil, err
		}
	}
	fmt.Println("Offers seeded successfully")
	if err := SaveIDsToFile(IDs, "to_delete/offer.txt"); err != nil {
		return nil, err
	}
	return IDs, nil
}
