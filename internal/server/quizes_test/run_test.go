...
func runInputFn(r *http.Request) (quizes.RunInput, error) {
	return quizes.RunInput{}, nil
}
...
func (r *runStorage) ParticipationStatus(ctx context.Context, userID string, quizID string) (shared.Participation, error) {
...
func TestGetRunInput(t *testing.T) {
	formValues := validRunFormValues()
	req := formRequest("GET", "/", formValues)
	input, err := quizes.GetRunInput(req)
	if err != nil {
		t.Error(err)
	}
...
func TestRunHandlerBadInput(t *testing.T) {
	invalidInputFn := func(r *http.Request) (quizes.RunInput, error) {
		return quizes.RunInput{}, errors.New("error")
	}
	handler := quizes.CreateRunHandler(
		&templates{},
		&runStorage{},
		&authRepo{},
		&streamService{},
