...
func TestLoginPageHandlerBadAuth(t *testing.T) {
	handler := profiles.CreateLoginPageHandler(&invalidAuthRepo{}, &templates{})
	if handler == nil {
...
func TestLoginPageHandlerBadTemplate(t *testing.T) {
	handler := profiles.CreateLoginPageHandler(&authRepo{}, &invalidTemplates{})
	if handler == nil {
...
func TestLoginPageHandler(t *testing.T) {
	handler := profiles.CreateLoginPageHandler(&authRepo{} ,&templates{})
	if handler == nil {
		t.Error("expected handler")
	}
	req, _ := http.NewRequest("GET", "/login", nil)
